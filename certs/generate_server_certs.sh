...
openssl req -new -x509 -days 3650 -key ./ca/ca.key -out ./ca/ca.crt -subj "/CN=${CN}/O=Elektronica-ICT Vives/C=BE"
...
openssl req -new -out ./server/server.csr -key ./server/server.key -subj "/CN=${CN}/O=Elektronica-ICT Vives/C=BE"
openssl x509 -req -in ./server/server.csr -CA ./ca/ca.crt -CAkey ./ca/ca.key -CAcreateserial -out ./server/server.crt -days 3650 -subj "/CN=server/O=Elektronica-ICT Vives/C=BE"

echo 'generating client key and csr in clinet folder...'
# Generate client certificate but don't sign. because clients 
openssl genrsa -out ./client/client.key 2048
openssl req -new -out ./client/client.csr -key ./client/client.key -subj "/CN=server_client/O=Elektronica-ICT Vives/C=BE"
